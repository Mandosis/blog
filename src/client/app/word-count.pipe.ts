...
export class WordCount implements PipeTransform {
  transform(input: string) {
    ...
    input = input.replace(/<(.|\n)*?>/g, ' '); // strip tags
    input = input.replace(nonANumLetters, ''); // ignore non-alphanumeric letters
    input = input.replace(/(^\s*)|(\s*$)/gi, ''); // exclude starting and ending white-space
    input = input.replace(/\n /gi, ' '); // convert newlines to spaces
    input = input.replace(/\n+/gi, ' ');
    input = input.replace(/[ ]{2,}/gi, ' '); // convert 2 or more spaces to 1
